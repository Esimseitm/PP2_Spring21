...
GRAY = (128, 128, 128)
# РОМБ 
def drawRhombus(screen, color, x, y, X1, Y1, radius):
    x1 = x
    x2 = X1
    y1 = y
    y2 = Y1
...
def drawRightTriangle(screen,color, x, y, X1, Y1, radius,):
    x1 = x
    x2 = X1
    y1 = y
    y2 = Y1
...
def drawEquilateralTriangle(surface,color, x, y, X1, Y1, radius):
    x1 = x
    x2 = X1
    y1 = y
    y2 = Y1
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if tool == 'rectangle':
                    ...
                    main_rect = True
                if tool == 'triangle':
                    drawEquilateralTriangle(screen,color, prev_pos[0], prev_pos[1],COORX[0], COORX[1] ,radius)
                if tool == 'phombus':
                    drawRhombus(screen,color, prev_pos[0], prev_pos[1], COORX[0], COORX[1], radius)
                if tool == 'righttriangle':
                    drawRightTriangle(screen,color, prev_pos[0], prev_pos[1], COORX[0], COORX[1], radius)
                if tool == 'circle':
