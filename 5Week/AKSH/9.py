...
#print(re.findall(r'\w+stan\b', text)) # все слова которые заканчиваются на СТАН

#print(re.findall(r'\d\d\.\d\d\.\d{2,4}', text))  # все года через точку

print(re.findall(r'[\+7|8]\(?\d{3}\)?-?\d{3}-\d{2}-\d{2}', text))   #все номера